...
# Load delimiter
delimiter  = js.document.getElementById("delimiter")
NAME_DELIM: str = delimiter.value if delimiter and delimiter.value else "@:@"

# Load input bin
from js import input_bin
soldier_pool: bytes = input_bin.to_bytes()

PATH_OUT = '/output.bin' # JS output bin
...
for line in namesTextArea.value.split('\n'): # Modified to read from textArea
...
################################
# Text below is copied from modify_lastname.py
################################
